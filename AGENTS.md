...
| 影视风格速查表 | `skills/VIDEO/references/film-style-lookup.md` |
| 小说转剧本 | `skills/VIDEO/小说转剧本/SKILL.md` |
| 剧本转分镜 | `skills/VIDEO/剧本转分镜/SKILL.md` |

---

## 🔴 技能联动规则（2026-08-06 新增·永久规则）

### 规则1：导演纠正 → 当场更新技能文件

导演纠正的错误/规则 → 立刻写进对应技能文件，不靠脑子记。

**执行：**
- 导演说「这里错了，应该是xxx」→ 识别错误类型 → 更新对应技能文件
- 导演说「这个规则记一下」→ 分类归属 → 写进对应文档

### 规则2：风格名 → 必须查表替换

导演说抽象风格名（「胡金铨风格」「张艺谋风格」）→ 必须查 `film-style-lookup.md` → 输出具体视觉参数，不直接写抽象风格名。

**执行：**
1. 导演说风格名
2. 立刻查 film-style-lookup.md
3. 把风格名替换成表中的「AI提示词关键词」

### 规则3：场景触发 → 自动联动技能组

识别场景类型，自动加载对应技能组，不等导演提醒。

**联动矩阵：**
| 场景类型 | 触发技能组 |
|---------|-----------|
| 打斗/战斗 | 打斗设计 + 动态运镜 + 传奇打斗 + 构图 + 运镜 + 真实感 + 场景一致性 + 色彩调色 + 视觉光效 |
| 对话/对峙 | 人物表演 + 镜头逻辑 + 构图 + 真实感 + 色彩调色 |
| 动作/运动 | 动态运镜 + 场景一致性 + 构图 + 环境生命力 |
| 远景/场景建立 | 构图 + 环境生命力 + 色彩调色 |
| 特写/表情 | 真实感增强 + 人物表演 + 镜头逻辑 |
| 光效/魔法 | 视觉光效基调 + 色彩调色 |
| 室内场景 | 场景质感增强 + 场景一致性 |
| 室外场景 | 环境生命力增强 + 色彩调色 |
...
今天建的技能，明天必须用。不是建完就忘，是要形成习惯。

**自我检查：**
- 每次出提示词前检查是否有相关技能/参考文件
- 每周检查技能库，识别建了没在用的技能，补上联动
